...
template<typename Dtype>
void EuclideanLossHeatmapLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
...
template <typename Dtype>
void EuclideanLossHeatmapLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
    Dtype loss = 0;

    int visualise_channel = 
        this->layer_param_.euclidean_loss_heatmap_param().visualise_channel();
    bool visualise = 
        this->layer_param_.euclidean_loss_heatmap_param().visualise();
      ...
      for (int idx_ch = 0; idx_ch < num_channels; idx_ch++) {
        for (int i = 0; i < label_height; i++) {
          for (int j = 0; j < label_width; j++) {
            int image_idx = idx_img * label_img_size + 
        ...
        } // end for -- label_height
      } // end for -- num_channels

      // Plot visualisation
      if (visualise) {
        // DLOG(INFO) << "num_images=" << num_images 
        //            << " idx_img=" << idx_img;
        // DLOG(INFO) << "sum bottom: " << cv::sum(bottom_img) 
        //            << "  sum gt: " << cv::sum(gt_img);
...
// visualisation_bottom: additional visualisation layer 
// (defined as the last 'bottom' in the loss prototxt def)
template <typename Dtype>
void EuclideanLossHeatmapLayer<Dtype>::Visualise(float loss, 
    cv::Mat bottom_img, cv::Mat gt_img, cv::Mat diff_img, 
    std::vector<cv::Point>& points, cv::Size size)
{
    DLOG(INFO) << loss;
...
template <typename Dtype>
void EuclideanLossHeatmapLayer<Dtype>::VisualiseBottom(
    const vector<Blob<Dtype>*>& bottom, 
    int idx_img, int visualise_channel, 
    std::vector<cv::Point>& points, cv::Size size)
{
  // Determine which layer to visualise
  Blob<Dtype>* visualisation_bottom = bottom[2];
  DLOG(INFO) << "visualisation_bottom: " 
             << visualisation_bottom->channels() 
             << " " << visualisation_bottom->height() << " " 
             << visualisation_bottom->width();

  // Format as RGB / gray
  bool isRGB = visualisation_bottom->channels() == 3;
  cv::Mat visualisation_bottom_img;
  if (isRGB) {
    visualisation_bottom_img = 
        cv::Mat::zeros(visualisation_bottom->height(), 
                       visualisation_bottom->width(), CV_32FC3);
  } else {
    visualisation_bottom_img = 
        cv::Mat::zeros(visualisation_bottom->height(), 
                       visualisation_bottom->width(), CV_32FC1);
  }
  ...
  for (int idx_ch = 0; idx_ch < visualisation_bottom->channels(); idx_ch++) {
    for (int i = 0; i < visualisation_bottom->height(); i++) {
      for (int j = 0; j < visualisation_bottom->width(); j++) {
        int image_idx = idx_img * visualisation_bottom->width() *
                        visualisation_bottom->height() * 
                        visualisation_bottom->channels() + 
                        idx_ch * visualisation_bottom->width() * 
                        visualisation_bottom->height() + 
                        i * visualisation_bottom->height() + j;
        if (isRGB && idx_ch < 3) {
          visualisation_bottom_img.at<cv::Vec3f>((int)j, (int)i)[idx_ch] = 
              4 * (float) visualisation_bottom->cpu_data()[image_idx] / 255;
        } else if (idx_ch == visualise_channel) {
          visualisation_bottom_img.at<float>((int)j, (int)i) = 
              (float) visualisation_bottom->cpu_data()[image_idx];
        }
      }
    }
  }
...
template <typename Dtype>
void EuclideanLossHeatmapLayer<Dtype>::Backward_cpu(
    ...
    const vector<Blob<Dtype>*>& bottom)
{
  ...
  // strictly speaking, should be normalising by (2 * channels) 
  // due to 1/2 multiplier in front of the loss
  Dtype loss = caffe_cpu_dot(count, diff_.cpu_data(), 
                             diff_.cpu_data()) / Dtype(channels);

  // copy the gradient
  memcpy(bottom[0]->mutable_cpu_diff(), diff_.cpu_data(), 
         sizeof(Dtype) * count);
  memcpy(bottom[1]->mutable_cpu_diff(), diff_.cpu_data(), 
         sizeof(Dtype) * count);
...
template <typename Dtype>
void EuclideanLossHeatmapLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
...
template <typename Dtype>
void EuclideanLossHeatmapLayer<Dtype>::Backward_gpu(
    const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, 
    const vector<Blob<Dtype>*>& bottom)
{
