...
template<typename Dtype>
void DataHeatmapLayer<Dtype>::DataLayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
{
  HeatmapDataParameter heatmap_data_param = 
  ...
  if(!sample_per_cluster_) {
    ...
    while (infile >> img_name >> labels >> cropInfos >> clusterClassStr) {
      ...
      while (ss) {
        ...
        if(labelinds_.empty() || std::find(labelinds_.begin(), 
           labelinds_.end(), labelCounter) != labelinds_.end())
        {
      ...
      img_label_list_.push_back(std::make_pair(img_name, 
                                std::make_pair(label, 
                                    std::make_pair(cropInfo, clusterClass))));
    }
    ...
    while (infile >> img_name >> labels >> cropInfos >> clusterClassStr) {
      ...
      img_list_[clusterClass].push_back(std::make_pair(img_name, 
                                        std::make_pair(label, 
                                          std::make_pair(cropInfo, clusterClass))));
    }
  ...
  if(!heatmap_data_param.has_meanfile()) {
    ...
    for (int n = 0; n < num_means_; n++) {
      ...
      LOG(INFO) << "per-video mean file array created: " << n << ": " 
                << mean_heights[n] << "x" << mean_widths[n] 
                << " (" << size << ")";
    }

    LOG(INFO) << "mean: " << mean_height << "x" << mean_width 
              << " (" << size << ")";
      ...
      for (int i = 0; i < mean_heights[n]; i++) {
        for (int j = 0; j < mean_widths[n]; j++) {
          for (int c = 0; c < this->datum_channels_; c++) {
              mean_img_[n].at<cv::Vec3f>(i, j)[c] =
                  mean_buf[meanOffset + ((n * this->datum_channels_ + c) * 
                          mean_height + i) * mean_width + j]; 
              //[c * mean_height * mean_width + i * mean_width + j];
          }
        }
      }
    }
  ...
  // init data
  this->transformed_data_.Reshape(batchsize, 
                                  this->datum_channels_, 
                                  outsize, outsize);
  top[0]->Reshape(batchsize, this->datum_channels_, outsize, outsize);
  for (int i = 0; i < this->PREFETCH_COUNT; ++i)
      this->prefetch_[i].data_.Reshape(batchsize, 
                                       this->datum_channels_, 
                                       outsize, outsize);
  ...
  top[1]->Reshape(label_batchsize, label_num_channels, 
                  label_height, label_width);
  for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
      this->prefetch_[i].label_.Reshape(label_batchsize, 
                                        label_num_channels, 
                                        label_height, label_width);
  }
...
void DataHeatmapLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  cv::Mat img, img_res, img_annotation_vis, img_mean_vis;
  cv::Mat img_vis, img_res_vis, mean_img_this, seg, segTmp;

  // Shortcuts to params
  const bool visualise   = heatmap_data_param.visualise();
  ...
  // For pose estimation with joints assumes i=0,1 are for head, 
  //   and i=2,3 left wrist, i=4,5 right wrist etc
  ...
  for (int idx_img = 0; idx_img < batchsize; idx_img++) {
    ...
    if(visualise) {
      ...
      this->VisualiseAnnotations(img_annotation_vis, 
                                 label_num_channels, 
                                 cur_label, multfact);
    ...
    int width    = img.cols;
    int height   = img.rows;
    int x_border = width  - size; // crop size
    ...
    */
    ...
    if(sub_mean) {
      ...
      std::string img_name_subdirImg = img_name.substr(
          img_name.find(delimiter) + 1, img_name.length());
      std::string meanIndStr = img_name_subdirImg.substr(
          0, img_name_subdirImg.find(delimiter));
      ...
      DLOG(INFO) << "Crop info: " << cur_cropinfo[0] << " " 
                 <<  cur_cropinfo[1] << " " << cur_cropinfo[2] 
                 << " " << cur_cropinfo[3] << " " << cur_cropinfo[4];
      DLOG(INFO) << "Crop info after: " << cur_cropinfo[0] << " " 
                 <<  cur_cropinfo[1] << " " << cur_cropinfo[2] << " " 
                 << cur_cropinfo[3] << " " << cur_cropinfo[4];
      DLOG(INFO) << "Mean image size: " << mean_img_this.cols << "x" 
                 << mean_img_this.rows;
      DLOG(INFO) << "Cropping: " << cur_cropinfo[0] - 1 << " " 
                 << cur_cropinfo[2] - 1 << " " << width << " " << height;

      // crop and resize mean image
      cv::Rect crop(cur_cropinfo[0] - 1, cur_cropinfo[2] - 1, 
                    cur_cropinfo[1] - cur_cropinfo[0], 
                    cur_cropinfo[3] - cur_cropinfo[2]);
    ...
    if(x_border < 0) {
      ...
      cv::copyMakeBorder(img, img, 0, 0, 0, -x_border, 
      ...
      if(visualise) {
        img_vis = img.clone();
        cv::copyMakeBorder(img_vis, img_vis, 0, 0, 0, -x_border, 
                           cv::BORDER_CONSTANT, cv::Scalar(0, 0, 0));
      }
    }
    ...
    for (int idx_aug = 0; idx_aug < num_aug; idx_aug++) {
      ...
      if(random_crop) {
        ...
        if(rand() % 2) {
          ...
          if(flip_joint_labels) {
            ...
            for (int i = flip_start_ind; i < label_num_channels; i += 4) {
              ...
              tmp_x = cur_label_aug[i + 0];
              tmp_y = cur_label_aug[i + 1];
              cur_label_aug[i + 0] = cur_label_aug[i + 2];
        ...
        // NOTE: no full copy performed, so the original image buffer 
        // is affected by the transformations below
        ...
        if(visualise) {
          ...
          this->VisualiseAnnotations(img_res_vis, label_num_channels, 
                                     cur_label_aug, multfact);
        ...
        for (int i = 0; i < label_num_channels; i += 2) {
          ...
          float x = cur_label_aug[i + 0] * (float) multfact;
          float y = cur_label_aug[i + 1] * (float) multfact;

          // rotate
          cur_label_aug[i + 0] = M.at<double>(0, 0) * x +
                                 M.at<double>(0, 1) * y + 
                                 M.at<double>(0, 2);
          cur_label_aug[i + 1] = M.at<double>(1, 0) * x + 
                                 M.at<double>(1, 1) * y + 
                                 M.at<double>(1, 2);
          // convert back to joint space
          cur_label_aug[i + 0] /= (float) multfact;
        ...
        // "crop" annotations
        for (int i = 0; i < label_num_channels; i += 2) {
          cur_label_aug[i + 0] -= (float)x0 / (float) multfact;
          cur_label_aug[i + 1] -= (float)y0 / (float) multfact;
        }
      ...
      for (int c = 0; c < channels; c++) {
        for (int i = 0; i < outsize; i++) {
          for (int j = 0; j < outsize; j++) {
            const int off2 = idx_img_aug * img_size + 
                             c * channel_size + i * outsize + j;
            top_data[off2] = img_res.at<cv::Vec3f>(i, j)[c];
          }
        }
      }
      ...
      for (int idx_ch = 0; idx_ch < label_num_channels / 2; idx_ch++) {
        float x = label_resize_fact * cur_label_aug[2 * idx_ch + 0] * multfact;
        float y = label_resize_fact * cur_label_aug[2 * idx_ch + 1] * multfact;
        for (int i = 0; i < label_height; i++) {
          for (int j = 0; j < label_width; j++) {
            int label_idx = idx_img_aug * label_img_size + 
                            idx_ch * label_channel_size  + 
                            i * label_height + j;
            float gaussian = (1 / (sigma * sqrt(2 * M_PI))) * 
                               exp(-0.5 * (pow(i - y, 2.0) + pow(j - x, 2.0)) * 
                                   pow(1 / sigma, 2.0));
            gaussian             = 4 * gaussian; // ???
...
template<typename Dtype>
void DataHeatmapLayer<Dtype>::GetCurImg(string& img_name, 
    std::vector<float>& img_label, std::vector<float>& crop_info, 
    int& img_class)
{
  if(!sample_per_cluster_) {
...
template<typename Dtype>
void DataHeatmapLayer<Dtype>::VisualiseAnnotations(
    cv::Mat img_annotation_vis, int label_num_channels, 
    std::vector<float>& img_class, int multfact)
{
  // colors
  ...
  for (int i = 0; i < label_num_channels; i += 2) {
      ...
      centers[coordInd] = cv::Point(img_class[i + 0] * multfact, 
                                    img_class[i + 1] * multfact);
      cv::circle(img_annotation_vis, centers[coordInd], 1, 
                 colors[coordInd], 3);
  }

  // connecting lines
  cv::line(img_annotation_vis, centers[1], centers[3], 
           CV_RGB(0, 255, 0), 1, CV_AA);
  cv::line(img_annotation_vis, centers[2], centers[4], 
           CV_RGB(255, 255, 0), 1, CV_AA);
  cv::line(img_annotation_vis, centers[3], centers[5], 
           CV_RGB(0, 0, 255), 1, CV_AA);
  cv::line(img_annotation_vis, centers[4], centers[6], 
...
// why not use reference `&` for param `src`
template <typename Dtype>
