    ...
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
  ...
  if(pose_pdj_accuracy_param.has_zero_iter_test()) {
    this->zero_iter_test_ = pose_pdj_accuracy_param.zero_iter_test();
  }
  // accuracy log file
    ...
    const Dtype* pred_coords_ptr, const Dtype* gt_coords_ptr) 
{
  ...
  const int hip_idx = this->hip_id_ * 2;
  const int sho_idx = this->shoulder_id_ * 2;

  // shoulder and hip indices&coordinates
  Dtype sho_x = gt_coords_ptr[sho_idx];
  Dtype sho_y = gt_coords_ptr[sho_idx + 1];
  Dtype hip_x = gt_coords_ptr[hip_idx];
  Dtype hip_y = gt_coords_ptr[hip_idx + 1];
  // if the ground truth is not valid, then ignore it
  if(sho_x < Zero || sho_y < Zero || hip_x < Zero || hip_y < Zero) return;

  const Dtype diff_x = sho_x - hip_x;
  const Dtype diff_y = sho_y - hip_y;
  ...
  for (int idx = 0; idx < this->label_num_; idx += 2) {
    ...
    // predicted x and y
    const Dtype pred_x = pred_coords_ptr[idx];
...
void PosePDJAccuracyLayer<Dtype>::WriteResults(const float total_accuracies[]) {
  ...
  acc_fhd << GlobalVars::SpiltCodeBoundWithStellate() << std::endl;
  for(int afn = 0; afn < this->acc_factor_num_; afn++) {
    acc_fhd << "afn: " << afn  << ", PDJ: " << acc_factors_[afn] 
        << ", max score: " << this->max_score_[afn] 
        << ", iter: " << this->max_score_iter_[afn] << std::endl;
    LOG(INFO) << "afn: " << afn  << ", PDJ: " << acc_factors_[afn] 
        << ", max score: " << this->max_score_[afn] 
...
void PosePDJAccuracyLayer<Dtype>::QuanFinalResults() {
  ...
  // save
  if(GlobalVars::caffe_iter() <= 0 && !this->zero_iter_test_) return;
  this->WriteResults(total_accuracies);
}

template<typename Dtype>
void PosePDJAccuracyLayer<Dtype>::Quantization(
    const Dtype* pred_coords, const Dtype* gt_coords, const int num) 
{
