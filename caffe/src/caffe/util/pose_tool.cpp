...
void get_parts_orders(const std::string source, std::vector<int>& o_parts_orders, 
    std::vector<int>& f_parts_orders) 
{
  ...
  for(int idx = 0; idx < num; idx++) {
    filer >> oidx >> fidx;
    CHECK(oidx >= 0 && oidx < num);
    CHECK(fidx >= 0 && fidx < num);
    osp.insert(oidx);
    o_parts_orders.push_back(oidx);
    fsp.insert(fidx);
    f_parts_orders.push_back(fidx);
  }
  CHECK(osp == fsp);
  CHECK(osp.size() == num);
  CHECK(o_parts_orders.size() == num);
  CHECK(f_parts_orders.size() == num);
  // CHECK_EQ(osp.size(), fsp.size());
  // CHECK(std::equal(osp.begin(), osp.begin() + osp.size(), fsp.begin()));
  
  LOG(INFO) << "parts orders' filepath: " << source;
  LOG(INFO) << "number of parts: " << num;
  for(int idx = 0; idx < num; idx++) {
    oidx = o_parts_orders[idx];
    fidx = f_parts_orders[idx];
    LOG(INFO) << "idx: " << idx << ", oidx: " << oidx << ", fidx: " << fidx;
  }
}
...
// index starts from zero
void get_parts_names(const std::string source, std::vector<std::string>& parts_names) {
  std::ifstream filer(source.c_str());
  CHECK(filer);
  LOG(INFO) << "parts names' filepath: " << source;

  int idx = 0;
  string part_name;
  // parts_names.clear();
  while (getline(filer, part_name)) {
    LOG(INFO) << "idx: " << idx << ", part name: " << part_name;
    idx++;
    parts_names.push_back(part_name);
  }
  CHECK_EQ(parts_names.size(), idx);
}

// index starts from zero
const std::vector<std::string> get_parts_names(const std::string source) {
  std::ifstream filer(source.c_str());
  CHECK(filer);
  LOG(INFO) << "parts names' filepath: " << source;
