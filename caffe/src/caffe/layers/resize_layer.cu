...
void ResizeLayer<Dtype>::Forward_gpu(
...
template <typename Dtype>
__global__ void kernel_ResizeBackward(
		const int nthreads, 
		...
		const Dtype* weight4)
{
	CUDA_KERNEL_LOOP(index, nthreads) {
		...
		for (int idx = 0; idx < top_step; ++idx) {
			// 
			bottom_diff[bottom_diff_offset + int(loc1[idx])] 
...
{
	if(!propagate_down[0]) {
		LOG(INFO) << this->layer_param_.name() << " does not need backward";
		return;
	}
	...
	kernel_ResizeBackward<Dtype> <<<CAFFE_GET_BLOCKS(loop_n), 
