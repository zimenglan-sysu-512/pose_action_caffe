    ...
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
{
  ...
  if(!this->start_skel_idxs_.empty())  {
    this->start_skel_idxs_.clear();
  }
  if(!this->end_skel_idxs_.empty()) {
    this->end_skel_idxs_.clear();
  }
  // index starts from zero
  get_skeleton_idxs(this->skel_path_, 
      this->start_skel_idxs_, this->end_skel_idxs_);
  CHECK(this->start_skel_idxs_.size() > 0);
  CHECK_EQ(this->start_skel_idxs_.size(), 
           this->end_skel_idxs_.size());
  LOG(INFO);
  ...
  if(!this->draw_joints_inds_.empty()) {
    this->draw_joints_inds_.clear();
  }
  if(visual_pose_coords_param.has_need_to_draw_inds_string()) {
    const std::string need_to_draw_inds_string = 
        visual_pose_coords_param.need_to_draw_inds_string();
    if(need_to_draw_inds_string.length()) {
      vector<string> info;
      boost::split(info, need_to_draw_inds_string, 
                   boost::is_any_of(","));
      for(int idx = 0; idx < info.size(); idx++) {
        this->draw_joints_inds_.push_back(std::atoi(info[idx].c_str()));
      }
    }
  }
  if(this->draw_joints_inds_.size()) {
    for(int j = 0; j < this->draw_joints_inds_.size(); j++) {
      LOG(INFO) << "ind (draw): " << this->draw_joints_inds_[j];
    }
  }
}
    ...
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
{
  ...
  const int channels = bottom[0]->count() / bottom[0]->num();
  CHECK_EQ(this->channels_, channels);
  ...
  const int n_parts = channels / 2;
  if(this->start_skel_idxs_.size()) {
    CHECK_EQ(this->start_skel_idxs_.size(), 
             this->end_skel_idxs_.size());
    for(int j = 0; j < this->start_skel_idxs_.size(); j++) {
      CHECK_LT(this->start_skel_idxs_[j], n_parts);
      CHECK_LT(this->end_skel_idxs_[j],   n_parts);
    }
  }

  if(this->draw_joints_inds_.size()) {
    for(int j = 0; j < this->draw_joints_inds_.size(); j++) {
      CHECK_LT(this->draw_joints_inds_[j], n_parts);
    }
  } else {
    for(int j = 0; j < n_parts; j++) {
      this->draw_joints_inds_.push_back(j);
    }
  }
}
    ...
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) 
{
  ...
  for(int item_id = 0; item_id < this->num_; item_id++) {
    ...
    for(int bs = 0; bs < b_size - 1; bs++) {
      ...
      //   // get point
      //   const Dtype x = bottom[bs]->data_at(item_id, idx + 0, 0, 0);
      ...
      // index starts from zero
      for(int j = 0; j < this->draw_joints_inds_.size(); j++) {
        const int ind = this->draw_joints_inds_[j];
        const int j2  = ind * 2; 
        const Dtype x = bottom[bs]->data_at(item_id, j2 + 0, 0, 0);
        const Dtype y = bottom[bs]->data_at(item_id, j2 + 1, 0, 0);
        cv::Point p(x, y);
        cv::circle(img, p, radius, colors[bs], thickness);

        if(this->is_draw_text_ && bs < 1) {
          const std::string text = to_string(j);
          if(j % 2) {
            cv::Point text_point(x - dx, y - dy);
            cv::putText(img, text, text_point, fontFace, fontScale, 
                        text_color_ood);
          } else {
            cv::Point text_point(x + dx, y + dy);
            cv::putText(img, text, text_point, fontFace, fontScale, 
    ...
    if(this->is_draw_skel_) { 
      ...
      for(int s_idx = 0; s_idx < this->start_skel_idxs_.size(); s_idx++) {
        ...
        // get point
        const Dtype sx = bottom[idx3]->data_at(item_id, si + 0, 0, 0);
        const Dtype sy = bottom[idx3]->data_at(item_id, si + 1, 0, 0);
        const cv::Point sp(sx, sy);
        const Dtype ex = bottom[idx3]->data_at(item_id, ei + 0, 0, 0);
