...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  for (int i = start; i <= end; ++i) {
    ...
    if(!this->first_iter_debug_) {
      LOG(INFO) << "forwarded layer name: " << layer_names_[i];
    }
  }

  this->first_iter_debug_ = true;
  return loss;
}
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    for (int j = 0; j < target_blobs.size(); ++j) {
      ...
      CHECK(target_blobs[j]->shape() == source_blob->shape()) 
          << "j: " << j 
          << ", target layer name: " <<  layer_names_[target_layer_id]
          << ", layer name: " << source_layer_name
          << ", target shape: " << target_blobs[j]->shape().size()
          << "("
          << target_blobs[j]->shape()[0] << ", "
          << target_blobs[j]->shape()[1] << ", "
          << target_blobs[j]->shape()[2] << ", "
          << target_blobs[j]->shape()[3] << ")"
          << ", source shape: " << source_blob->shape().size()
          << "("
          << source_blob->shape()[0] << ", "
          << source_blob->shape()[1] << ", "
          << source_blob->shape()[2] << ", "
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    int target_layer_id = 0;
    int target_layer_copy_postfix_id = 0;
    std::vector<int> target_layer_copy_postfix_ids;
    ...
    // 
    if(target_layer_id == layer_names_.size()) {
      while (target_layer_copy_postfix_id != layer_names_.size()) {
        const bool has_copy_postfix_name = 
            layers_[target_layer_copy_postfix_id]->layer_param().has_copy_postfix_name();
        // 
        if(has_copy_postfix_name) {
          const string& target_layer_name = layer_names_[target_layer_copy_postfix_id];
          const string copy_postfix_name = 
              layers_[target_layer_copy_postfix_id]->layer_param().copy_postfix_name();

          // Check whether source_layer_name endwiths copy_postfix_name
          const int tlen = target_layer_name.length();
          const int clen = copy_postfix_name.length();
          if(tlen <= clen) {
            ++target_layer_copy_postfix_id;
            continue;
          }
          // 
          const string& sub_copy_postfix_name = 
            target_layer_name.substr(tlen - clen, clen);
          // 
          if(source_layer_name == copy_postfix_name &&
              source_layer_name == sub_copy_postfix_name) {
            // may be has multi-postfix, like primary_fc6, secondary_fc6
            target_layer_copy_postfix_ids.push_back(target_layer_copy_postfix_id);
          }
        }
    ...
    const bool has_target = target_layer_copy_postfix_ids.size();
    if (target_layer_id == layer_names_.size() && has_target <= 0) {
    ...
    // 
      ...
      for(int tlcpi = 0; tlcpi < target_layer_copy_postfix_ids.size(); tlcpi++) {
        const int target_layer_id2 = target_layer_copy_postfix_ids[tlcpi];
        LOG(INFO) << "Copying source layer (" << source_layer_name 
            << ") from pretrained layer to the target layer (" 
            << layer_names_[target_layer_id2] << ")";
        // 
        vector<shared_ptr<Blob<Dtype> > >& target_blobs =
            layers_[target_layer_id2]->blobs();
        CHECK_EQ(target_blobs.size(), source_layer.blobs_size())
            << "Incompatible number of blobs for layer " << source_layer_name;
        for (int j = 0; j < target_blobs.size(); ++j) {
          const bool kReshape = false;
