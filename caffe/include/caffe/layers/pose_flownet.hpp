...
#ifndef CAFFE_POSE_FLOWNET_HPP_
#define CAFFE_POSE_FLOWNET_HPP_
...
paper:  Flowing ConvNets for Human Pose Estimation in Videos
...
class DataHeatmapLayer: public BasePrefetchingDataLayer<Dtype> {
 ...
 protected:
  ...
  inline void GetCurImg(string& img_name, std::vector<float>& img_class, 
                        std::vector<float>& crop_info, int& cur_class);
  inline void AdvanceCurImg();
  // Visualise point annotations
  inline void VisualiseAnnotations(cv::Mat img_annotation_vis, 
                                   int numChannels, 
                                   std::vector<float>& cur_label, int width);
  ...
  vector< vector< pair<string, pair<vector<float>, 
                  pair<vector<float>, int> > > > > img_list_;

  // vector of (image, label) pairs
  vector< pair<string, pair<vector<float>, pair<vector<float>, 
                                                int> > > > img_label_list_;    
};
...
protected:
  ...
  virtual void Visualise(float loss, cv::Mat bottom_img, 
                         cv::Mat gt_img, cv::Mat diff_img, 
                         std::vector<cv::Point>& points, cv::Size size);
  virtual void VisualiseBottom(const vector<Blob<Dtype>*>& bottom, 
  ...
  bool val_mode_;       // validation mode flag
};

}

#endif /* CAFFE_POSE_FLOWNET_HPP_ */