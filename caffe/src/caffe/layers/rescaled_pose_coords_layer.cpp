      ...
      const vector<Blob<Dtype>*>& top) 
{
  ...
  for(int item_id = 0; item_id < this->num_; item_id++) {
    ...
    for(int idx = 0; idx < this->channels_; idx += 2) {
      const Dtype x = bootom_coords[offset] / im_scale;
      top_coords[offset] = x >= img_width ? nOne : (flippable ? img_width - 1 - x : x);
      offset++;

      const Dtype y = bootom_coords[offset] / im_scale;
      top_coords[offset] = y >= img_height ? nOne : y;
      offset++;
    }
  }
