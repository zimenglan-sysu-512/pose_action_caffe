  ...
  for(int item_id = 0; item_id < this->num_; item_id++) {
    ...
    for(int bs = 0; bs < b_size - 1; bs++) {
      ...
      for(int part_idx = 0; part_idx < this->channels_; part_idx += 2) {
        Dtype x = bottom[bs]->data_at(item_id, part_idx + 0, 0, 0) / im_scale;
        x = flippable ? img_width - x - 1 : x;
        Dtype y = bottom[bs]->data_at(item_id, part_idx + 1, 0, 0) / im_scale;
        filer << x << " " << y << std::endl;
      }
    }
  }
  ...
  for(int item_id = 0; item_id < this->num_; item_id++) {
    ...
    for(int bs = 0; bs < b_size - 1; bs++) {
      for(int idx = 0; idx < this->channels_; idx +=2) {
        // get point
        const Dtype x = bottom[bs]->data_at(item_id, idx, 0, 0) / im_scale;
        const Dtype y = bottom[bs]->data_at(item_id, idx + 1, 0, 0) / im_scale;
        cv::Point p(x, y);
...
#ifdef CPU_ONLY
STUB_GPU(VisualizedPoseCoords2Layer);
#endif

INSTANTIATE_CLASS(VisualizedPoseCoords2Layer);
REGISTER_LAYER_CLASS(VisualizedPoseCoords2);
