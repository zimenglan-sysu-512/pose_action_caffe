...
mkdir -p $ExperPath
# models/Pose/FLIC/black/lecun-8x-2b-sd5_5-marks-dsn/solver_main.prototxt
solver_main_proto="solver_main.prototxt"
solver_main_proto=$ExperPath$solver_main_proto
echo $solver_main_proto
# ../asserts/models/Pose/FLIC/black/lecun-8x-2b-sd5_5-marks-dsn/
ModelsDir="../asserts/"
ModelsDir=$ModelsDir$ExperPath
mkdir -p $ModelsDir
# ../asserts/models/Pose/FLIC/black/lecun-8x-2b-sd5_5-marks-dsn/models.main/
...
if [ ! -n "$1" ] ;then
	re_iter=0
	# run & log command
	$caffe_bin train --solver=$solver_main_proto 2>&1 | tee -a $log_filename
else
	re_iter=$1
	resume_model_file="flic_iter_"$re_iter".solver_mainstate"
	...
	echo
	# run & log command
	$caffe_bin train --solver=$solver_main_proto --snapshot=$resume_model_file 2>&1 | tee -a $log_filename
fi
