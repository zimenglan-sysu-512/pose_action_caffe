...
void VisualizedPoseCoordsLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void VisualizedPoseCoordsLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) 
{
...
template <typename Dtype>
void VisualizedPoseCoordsLayer<Dtype>::WriteFiles(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) 
{
  const int b_size = bottom.size();
...
template <typename Dtype>
void VisualizedPoseCoordsLayer<Dtype>::WriteImages(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) 
{
  const int b_size = bottom.size();
  ...
  for(int item_id = 0; item_id < this->num_; item_id++) {
    ...
    for(int s_idx = 0; s_idx < this->start_skel_idxs_.size(); s_idx++) {
      ...
      // get point
      const Dtype sx = bottom[b_size - 2]->data_at(item_id, start_idx * 2, 0, 0);
      const Dtype sy = bottom[b_size - 2]->data_at(item_id, start_idx * 2 + 1, 0, 0);
      const cv::Point sp(sx, sy);
      const Dtype ex = bottom[b_size - 2]->data_at(item_id, end_idx * 2, 0, 0);
      const Dtype ey = bottom[b_size - 2]->data_at(item_id, end_idx * 2 + 1, 0, 0);
...
template <typename Dtype>
void VisualizedPoseCoordsLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) 
{
...
template <typename Dtype>
void VisualizedPoseCoordsLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
