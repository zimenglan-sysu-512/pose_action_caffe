...
mkdir -p $ExperPath
# models/Pose/FLIC/black/lecun-8x-2b-sd5_5-marks-dsn/solver_side.prototxt
solver_side_proto="solver_side.prototxt"
solver_side_proto=$ExperPath$solver_side_proto
echo $solver_side_proto
# ../asserts/models/Pose/FLIC/black/lecun-8x-2b-sd5_5-marks-dsn/
ModelsDir="../asserts/"
ModelsDir=$ModelsDir$ExperPath
mkdir -p $ModelsDir
# ../asserts/models/Pose/FLIC/black/lecun-8x-2b-sd5_5-marks-dsn/models.side/
model_path="models.side/"
...
if [ ! -n "$1" ] ;then
	...
	echo
	# run & log command
	$caffe_bin train --solver=$solver_side_proto --snapshot=$resume_model_file 2>&1 | tee -a $log_filename
fi
